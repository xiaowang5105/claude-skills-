...
4. 先执行“文献关系与文献充分性关”：凡涉及 2 篇及以上文献，检查是否有文献关系表，是否说明并列、递进、冲突、补充、机制承接或情境差异，且这些关系是否进入正文。
5. 检查现有文献是否足以支撑研究问题、机制链、研究缺口和贡献定位；若不足且 writer 没有按顶刊优先级补文献，判为不通过。
6. 按 `workflow-rules.md` 执行 13 条结构/语言/忠实检查；没有 A 档模板不影响检查，不得因缺少模板放宽或阻断检查。
7. 按需读取 `material-tier-index.md` 和 A/B/C 素材作参照：A 是高质量摘录，可参照完整结构；B 是可用摘录，只参照去事实后的骨架；C 是原始/低质量材料，只参照词感；素材参照不得决定通过。
8. 执行素材误用检查：是否把 B 档作者、年份、政策、数据、案例、具体结论当事实依据；是否把 C 档 raw-ocr 当结构模板；是否模仿空括号、缺年份、残缺英文、错断句或页眉页脚。
9. 逐条核对现实背景事实：政策、年份、事件、数据、行业事实、企业案例、制度背景或国际形势。
10. 逐条核对文献引用准确性：方向一致、内容一致、归属正确。
11. 检查逻辑链是否完整：现实背景 -> 研究问题 -> 文献群落 -> 缺口 -> 本文贡献。
12. 按 `workflow-rules.md` 的报告格式输出检查报告；报告必须标注问题权重和验证层级，不通过时附可直接带回 `hexie-writer` 的句段级修正清单。
...
- 忠实：词汇适配、删引用后逻辑连贯性。
- 文献关系：文献群落、关系类型、文献充分性、补文献必要性、关系是否进入正文。
...
- 文献关系与文献充分性关、13 项检查、现实背景核验、文献引用准确性全部通过，才可判定“通过”。
- 任一检查项不通过，判定“不通过”。
...
报告必须包含：检查范围、组织方式、检查结论、材料状态、文献关系与文献充分性、检查清单、问题权重、验证层级、现实背景核验、文献引用准确性、素材误用检查、语料参照、违规详情、修正清单、阻塞项和被检查文本。

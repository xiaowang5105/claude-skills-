...
循环带有硬停止条件：每一轮最多定点修正 2 次；同一轮第 2 次仍不通过时停止循环并输出阻塞项。事实、引用或材料问题不能靠语言润色绕过。

材料不足时，Hexie 可以继续给出结构和语言诊断，也可以输出“待核验稿”；但不能判定终稿通过，不能把待验证引用或背景事实当作已验证事实处理。
...
8. 自动运行 `hexie-check` 三轮检查，并根据每轮修正清单定点修改；每轮最多重跑 2 次，直到三轮全部通过或触发阻塞。

`hexie-writer` 的正常终点只有一个：`hexie-check` 全部通过且无待验证项。  
事实无法核验、材料缺失、引用无法确认或背景来源不足时，writer 必须暂停并列出阻塞项；若继续输出文字，只能标注为“待核验稿”。
...
- 每一轮最多重跑 2 次；第 2 次仍不通过时输出阻塞项，不继续循环。
- 材料不足模式下，第二、三轮可以作为附加诊断继续执行，但总判定仍为“待验证”，输出状态只能是“待核验稿”。

## Writer-Check Loop

Hexie 的自动闭环由 `hexie-writer` 控制，不由 `hexie-check` 自己循环。三轮顺序固定，不能跳；每轮最多重跑 2 次，第 2 次仍不通过就停止并输出阻塞项。

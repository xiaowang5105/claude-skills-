...
- `hexie-shared-resource-library`：共享资源库，保存模板、语料、提取质量分层和 writer/check 共用规则。
...
若现有文献不足，按国内五大、国外顶刊、NBER/CEPR/SSRN 优先级补文献
        ↓
writer 第一次写作或改写，并把文献关系写进正文
        ↓
writer 进入 hexie-check
        ↓
check 先检查“文献关系与充分性”是否过关
        ↓
阶段 2：文本质量关
        ↓
check 检查结构 / 语言 / 现实背景 / 引用准确性 / 素材误用
        ↓
...
凡涉及 2 篇及以上文献，第一关必须先检查文献关系和文献充分性。若文献关系没有进入正文，或现有文献不足却没有补文献，不能只做语言润色，也不能判定通过。
...
- `material-tier-index.md` 规定“写法类型 × 提取质量”的二维索引。
- `workflow-rules.md` 保存适用范围、事实来源优先级、八种组织方式、writer-check 循环、检查权重、13 条检查清单和报告格式。
- `tier-a-complete-structure-templates/` 是 A 档高质量摘录入口；只放人工截出的干净完整开篇段落。
- `tier-b-skeleton-structure-templates/` 是 B 档可用摘录入口；当前 01/02/03/04/05/06/08 主要在这里。
- `tier-c-style-corpus/` 是 C 档原始/低质量材料入口；当前 07、历史大素材和 MinerU/OCR 原始材料在这里。

## Material Tiers

素材库采用二维结构：8 种写法是横向分类，A/B/C 是纵向提取质量等级。每一种写法理论上都可以有 A、B、C 三档素材；A/B/C 不代表写法本身好坏，也不评价原论文质量，只决定“这份摘录现在能被 AI 怎么安全使用”。

| Tier | Name | Can be used for | Cannot be used for |
|------|------|-----------------|--------------------|
| A | 高质量摘录 | 干净完整开篇摘录的整段结构、问题链、文献群落、贡献定位、句层节奏、收束方式 | 当前论文事实依据；照搬原文；模仿具体年份、政策、数据、案例或作者结论 |
| B | 可用摘录 | 删除作者、年份、政策、数据、案例、具体结论后的判断链、句式节奏、连接方式 | 引用事实、背景事实、整段原样模仿 |
| C | 原始/低质量材料 | 词语、短语、连接方式、中文经管论文气口 | 结构模板、引用事实、背景事实、标点和引用格式 |
